...
class SettingsView:
    ...
    def _button_style(self, bgcolor: str, color: str | None = None) -> ft.ButtonStyle:
    ...
    def _apply_theme(self, name: str) -> None:
        actual = T.os_preferred_theme() if name == "sistema" else name
        T.apply_theme(actual)
        T.save_app_theme(name)
    ...
    def _theme_card(self, name: str, label: str, icon, bg: str, text_color: str, accent: str) -> ft.Control:
        saved = self.user_data.get("theme", T._current)
        selected = saved == name
        return ft.Container(
            expand=True,
            bgcolor=bg,
            border=ft.border.all(2.5 if selected else 1.5, accent if selected else T.BORDER),
            border_radius=T.RADIUS,
            padding=ft.padding.all(16),
            on_click=lambda e: self._apply_theme(name),
            ink=True,
            content=ft.Column(
                [
                    ft.Icon(icon, color=accent, size=28),
                    ft.Text(label, size=13, color=text_color, weight=ft.FontWeight.W_600, font_family=T.FONT_DISPLAY),
                    ft.Container(
                        width=32, height=3, bgcolor=accent,
                        border_radius=2,
    ...
    def build(self) -> ft.Control:
        ...
        appearance_panel = self._panel(
            "Aspetto",
            ft.Icons.PALETTE_ROUNDED,
            [
                ft.Text("Scegli il tema dell'app.", size=12, color=T.TEXT_M, font_family=T.FONT_BODY),
                ft.Row(
                    [
                        self._theme_card(
                            "murasaki", "Viola Murasaki",
                            ft.Icons.DARK_MODE_ROUNDED,
                            "#1E1630", "#EDE8FF", "#C9A0DC",
                        ),
                        self._theme_card(
                            "fuji", "Fuji Glicine",
                            ft.Icons.LIGHT_MODE_ROUNDED,
                            "#E3D8F2", "#1E1030", "#7B52A8",
                        ),
                        self._theme_card(
                            "sistema", "Sistema",
                            ft.Icons.BRIGHTNESS_AUTO_ROUNDED,
                            T.BG_SURF, T.TEXT, T.PURPLE,
                        ),
                    ],
                    spacing=12,
                    expand=True,
