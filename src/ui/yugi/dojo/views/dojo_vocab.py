"""
ui/yugi/dojo/views/dojo_vocab.py – Addestramento Vocabolario (Dojo)
...
class DojoVocab:
    def __init__(self, page: ft.Page, navigate, state: dict):
        ...
        raw_data = DBManager.load_json("vocabolario.json") or []
        self.vocab_pool = [(i["word"], i["meaning"], i.get("group", "Generale")) for i in raw_data if "word" in i and "meaning" in i]
        
        groups_set = {v[2] for v in self.vocab_pool}
    ...
    def _setup_screen_group(self) -> ft.Control:
        def make_special_all_card() -> ft.Container:
            def on_hover(e):
                ...
                e.control.border = ft.border.all(1.5, T.BELT_VOCAB)
                e.control.bgcolor = f"{T.BELT_VOCAB}11" if is_hover else T.BG_SURF
                e.control.update()
            ...
            return ft.Container(
                content=ft.Row([
                    ft.Container(
                        ...
                        border=ft.border.all(1.5, T.BELT_VOCAB), border_radius=6,
                        content=ft.Text("全", size=20, font_family=T.FONT_DISPLAY, color=T.BELT_VOCAB, weight=ft.FontWeight.W_700),
                    ),
                    ft.Column([
                        ft.Text("Tutto il vocabolario", size=15, font_family=T.FONT_DISPLAY, weight=ft.FontWeight.W_700, color=T.TEXT),
                        ft.Text(f"全部 — {len(self.vocab_pool)} parole", size=11, font_family=T.FONT_DISPLAY, italic=True, color=T.BELT_VOCAB),
                    ], spacing=1, expand=True),
                    ft.Icon(ft.Icons.ARROW_FORWARD_IOS_ROUNDED, color=T.BELT_VOCAB, size=14),
                ], spacing=16, vertical_alignment=ft.CrossAxisAlignment.CENTER),
                bgcolor=T.BG_SURF, border_radius=T.RADIUS, border=ft.border.all(1, T.BELT_VOCAB),
        ...
        def make_group_card(g_name):
            def on_hover(e):
                is_hover = e.data == "true"
                e.control.border = ft.border.all(1.5, T.BELT_VOCAB if is_hover else T.BORDER)
                e.control.bgcolor = f"{T.BELT_VOCAB}0A" if is_hover else T.BG_CARD
                e.control.update()

            count = len([v for v in self.vocab_pool if v[2] == g_name])
            return ft.Container(
                content=ft.Column([
                    ft.Text(g_name.capitalize(), size=14, font_family=T.FONT_DISPLAY, weight=ft.FontWeight.W_700, color=T.TEXT),
                    ft.Container(expand=True),
                    ft.Row([
                        ft.Text(f"{count} parole", size=13, color=T.TEXT_M),
                        ft.Container(expand=True),
                        ft.Icon(ft.Icons.ARROW_FORWARD_IOS_ROUNDED, color=T.BELT_VOCAB, size=12, opacity=0.4),
        ...
        return ft.Column([
            ft.Text("Seleziona la bacheca del Vocabolario", size=18, font_family=T.FONT_DISPLAY, weight=ft.FontWeight.W_700, color=T.BELT_VOCAB),
    ...
    def _start_quiz_with_group(self, group_name):
        ...
        pool = self.vocab_pool if group_name == "Tutti" else [v for v in self.vocab_pool if v[2] == group_name]
        
        if not pool: return

        all_meanings = list({v[1] for v in self.vocab_pool})
        ...
        self.questions = []
        for word, correct, _ in target_qs:
            opts = make_choice_options(correct, all_meanings, placeholder="—", case_sensitive=False)
            self.questions.append((word, correct, opts))
    ...
    def _show_question(self):
        ...
        word, correct, options = self.questions[self.current_idx]
    ...
    def _show_results(self):
        ...
        grade, color, icon = ("Perfezione", T.BELT_VOCAB, "極") if pct == 100 else ("Ottimo lavoro", T.TEXT, "良") if pct >= 80 else ("Devi allenarti", T.RED, "学")
        ...
        screen = ft.Column([
            ...
            ft.Row([
                ft.Button("Riprova bacheca", style=ft.ButtonStyle(bgcolor=T.BG_CARD, color=T.TEXT), on_click=lambda e: self._start_quiz_with_group(self.selected_group)),
                ft.Button("Torna alle scelte", style=ft.ButtonStyle(bgcolor=T.BELT_VOCAB, color=T.BG_MAIN), on_click=lambda e: self._go_back_to_mode()),
    ...
    def build(self) -> ft.Control:
        if not self.vocab_pool:
            self.content_area.content = build_quiz_data_error("vocabolario.json")
        else:
            self.content_area.content = centered_stage(self.page, self._setup_screen_group(), max_width=1040)
        masthead = ft.Container(
            content=ft.Row([
                ...
                ft.Column([
                    ft.Text("Allenamento Vocabolario", size=T.FS_TITLE, font_family=T.FONT_DISPLAY, weight=ft.FontWeight.W_700, color=T.TEXT),
                    ft.Text("語彙 – Goi", size=T.FS_SMALL, font_family=T.FONT_DISPLAY, italic=True, color=T.BELT_VOCAB),
                ], spacing=2),
            ], spacing=16, vertical_alignment=ft.CrossAxisAlignment.CENTER),
