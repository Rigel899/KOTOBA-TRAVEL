...
class DashboardView:
    ...
    def __init__(self, page: ft.Page, navigate, state: dict):
        ...
        saved = self.user_data.get("theme")
        if saved is None:
            # Primo accesso: salva il tema corrente (caricato all'avvio)
            data = dict(self.user_data)
            data["theme"] = T._current
            DBManager.update_user_data(self.username, data)
        elif saved == "sistema":
            T.apply_theme(T.os_preferred_theme())
        else:
            T.apply_theme(saved)
    ...
    def _show_logo_preview(self, e=None) -> None:
        ...
        dialog = ft.AlertDialog(
            ...
            title=ft.Row(
                [
                    icon_btn(ft.Icons.ARROW_BACK_ROUNDED, icon_color=T.TEXT_M, icon_size=22, tooltip="Torna alla dashboard", on_click=close_dialog),
    ...
    def _trophy_state(self) -> tuple[int, str, str, int | None]:
        ...
        levels = [
            (50, "Platino", T.RARITY_PLATINO),
            (25, "Oro",     T.GOLD),
            (15, "Argento", T.RARITY_SILVER),
            (5,  "Bronzo",  T.RARITY_BRONZE),
            (1,  "Legno",   T.RARITY_WOOD),
            (0,  "Trofei",  T.TEXT_M),
        ]
    ...
    def _maybe_show_onboarding(self) -> None:
        ...
        dialog = ft.AlertDialog(
            ...
            content=ft.Container(
                ...
                content=ft.Column(
                    [
                        ...
                        ft.Row(
                            [
                                ft.ElevatedButton("Accademia", icon=ft.Icons.AUTO_STORIES_ROUNDED, style=ft.ButtonStyle(bgcolor=T.BELT_ACCADEMIA, color=T.TEXT, mouse_cursor=ft.MouseCursor.CLICK), on_click=lambda e: go("study")),
                                ft.ElevatedButton("Yugi", icon=ft.Icons.SPORTS_MARTIAL_ARTS_ROUNDED, style=ft.ButtonStyle(bgcolor=T.RED, color=T.TEXT, mouse_cursor=ft.MouseCursor.CLICK), on_click=lambda e: go("yugi")),
                                ft.ElevatedButton("Achievement", icon=ft.Icons.MILITARY_TECH_ROUNDED, style=ft.ButtonStyle(bgcolor=T.BG_SURF, color=T.TEXT, mouse_cursor=ft.MouseCursor.CLICK), on_click=lambda e: go("achievements")),
                            ],
                            spacing=10,
                            wrap=True,
                        ),
            ...
            actions=[
                ft.TextButton("Resto qui", style=ft.ButtonStyle(color=T.TEXT_M, mouse_cursor=ft.MouseCursor.CLICK), on_click=close_dialog),
            ],
        )
        self._open_dialog(dialog)

    def _toggle_theme(self, e=None) -> None:
        new_theme = "fuji" if T._current == "murasaki" else "murasaki"
        T.apply_theme(new_theme)
        self.page.bgcolor = T.BG_MAIN
        self.page.theme_mode = ft.ThemeMode.DARK if T.IS_DARK else ft.ThemeMode.LIGHT
        data = DBManager.get_user_data(self.username) or {}
    ...
    def _confirm_logout(self, e=None) -> None:
        ...
        dialog = ft.AlertDialog(
            ...
            actions=[
                ft.TextButton("Annulla", style=ft.ButtonStyle(color=T.TEXT_M, mouse_cursor=ft.MouseCursor.CLICK), on_click=close_dialog),
                ft.ElevatedButton("Disconnetti", style=ft.ButtonStyle(bgcolor=T.RED, color=T.TEXT, mouse_cursor=ft.MouseCursor.CLICK), on_click=do_logout),
            ],
        )
        self._open_dialog(dialog)
        ...
        return ft.Container(
            content=ft.Row(
                [
                    ...
                    ft.Container(expand=True),
                    icon_btn(ft.Icons.MILITARY_TECH_ROUNDED, icon_color=T.GOLD, icon_size=24, tooltip="Achievement", on_click=lambda e: self.navigate("achievements")),
                    icon_btn(ft.Icons.INSIGHTS_ROUNDED, icon_color=T.INDIGO, icon_size=24, tooltip="Statistiche", on_click=lambda e: self.navigate("stats")),
                    icon_btn(ft.Icons.SETTINGS_ROUNDED, icon_color=T.TEXT_M, icon_size=23, tooltip="Impostazioni", on_click=lambda e: self.navigate("settings")),
                    icon_btn(ft.Icons.LOGOUT_ROUNDED, icon_color=T.TEXT_M, icon_size=23, tooltip="Disconnetti", on_click=self._confirm_logout),
                ],
        ...
        travel_grid = ft.Container(
            ...
            content=ft.Column(
                [
                    ft.Row(
                        [
                            self._travel_link("Cibo", "和食 - Washoku", ft.Icons.RAMEN_DINING_ROUNDED, T.BELT_KANA, "food"),
                            self._travel_link("Storia", "歴史 - Rekishi", ft.Icons.HISTORY_EDU_ROUNDED, T.BELT_VOCAB, "history"),
                        ],
                        spacing=self._scaled(18),
                    ),
                    ft.Row(
                        [
                            self._travel_link("Cultura", "文化 - Bunka", ft.Icons.TEMPLE_BUDDHIST_ROUNDED, T.BELT_CULTURE, "culture"),
                            self._travel_link("Luoghi", "名所 - Meisho", ft.Icons.MAP_ROUNDED, T.BELT_KANJI, "places"),
                        ],
                        spacing=self._scaled(18),
                    ),
                ],
