...
class StatsView:
    ...
    def __init__(self, page: ft.Page, navigate, state: dict):
        ...
        self.EXPLORATION_ITEMS = [
            ("Cibo", "food_viewed", T.BELT_KANA, ft.Icons.RAMEN_DINING_ROUNDED),
            ("Luoghi", "places_viewed", T.BELT_KANJI, ft.Icons.MAP_ROUNDED),
            ("Cultura", "culture_viewed", T.BELT_CULTURE, ft.Icons.TEMPLE_BUDDHIST_ROUNDED),
            ("Storia", "history_viewed", T.BELT_VOCAB, ft.Icons.HISTORY_EDU_ROUNDED),
        ]
    ...
    def build(self) -> ft.Control:
        ...
        metrics = ft.Row(
            [
                ...
                self._metric("Domande corrette", str(total_correct), ft.Icons.CHECK_CIRCLE_ROUNDED, T.BELT_GRAMMAR),
                self._metric("Accuracy", f"{accuracy}%", ft.Icons.PERCENT_ROUNDED, T.BELT_VOCAB),
                self._metric("Streak massima", str(max_streak), ft.Icons.LOCAL_FIRE_DEPARTMENT_ROUNDED, T.BELT_KANJI),
            ],
            spacing=14,
        )
