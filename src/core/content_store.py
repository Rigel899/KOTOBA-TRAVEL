...
class ContentStore:
    ...
    def __init__(
        ...
        asset_dir: Callable[[], str] | None = None,
        cache: dict[str, tuple[int, object]] | None = None,
    ) -> None:
        self._data_dir = data_dir
        self._asset_dir = asset_dir or (lambda: os.path.dirname(data_dir()))
    ...
    def _validate_image_field(self, filename: str, index: int, item: dict, field: str) -> bool:
        ...
        if not os.path.exists(full_path):
            _log.debug("asset missing for %s[%s].%s: %s", filename, index, field, image_path)
        return True
